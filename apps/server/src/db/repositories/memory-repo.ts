...
export const memoryRepo = {
  create(data: { agent_id: string; type: string; content: string; source?: string }): Memory {
    ...
    db.prepare(
      `INSERT INTO memories (id, agent_id, type, content, source, relevance, accessed_at, created_at)
       VALUES (?, ?, ?, ?, ?, 1.0, ?, ?)`
  ...
  findByAgent(agentId: string, type?: string, limit = 100): Memory[] {
    ...
    if (type) {
      return db.prepare(
        `SELECT * FROM memories WHERE agent_id = ? AND type = ? ORDER BY relevance DESC, accessed_at DESC LIMIT ?`
      ).all(agentId, type, limit) as Memory[];
    }
    return db.prepare(
      `SELECT * FROM memories WHERE agent_id = ? ORDER BY relevance DESC, accessed_at DESC LIMIT ?`
  ...
  updateRelevance(id: string, relevance: number): void {
    const db = getDb();
    db.prepare(`UPDATE memories SET relevance = ? WHERE id = ?`).run(relevance, id);
  },

  touch(id: string): void {
    const db = getDb();
    const now = new Date().toISOString();
    db.prepare(`UPDATE memories SET accessed_at = ? WHERE id = ?`).run(now, id);
  },

  decayAll(agentId: string, factor = 0.9): void {
    const db = getDb();
    db.prepare(`UPDATE memories SET relevance = relevance * ? WHERE agent_id = ?`).run(factor, agentId);
  },

  prune(agentId: string, minRelevance = 0.1): number {
    const db = getDb();
    const result = db.prepare(
      `DELETE FROM memories WHERE agent_id = ? AND relevance < ?`
